...
import { cart } from 'src/assets/model';
...
export class ProductListComponent implements OnInit {
  ...
  constructor(private listService: RaavanTeamService, private router: Router,private logger: LoggingService, private route: ActivatedRoute, private us: RaavanTeamService, private cart: CartService) {
  ...
  ngOnInit(): void {
    ...
    if (this.subCategory == 'Electronics' || this.subCategory == 'Games' || this.subCategory == 'Home' || this.subCategory == 'Fashion' || this.subCategory == 'Books' || this.subCategory == 'Essentials') {
      this.listService.getAllByCategory(this.subCategory).subscribe(
        data => {
          this.logger.logStatus('Got all the wishlisted items succesfully');
          this.handler(data);
        }
      )
    }
  ...
  getWishlists() {
    this.us.getAllWishlists("harsha").subscribe(data1 => {  this.wishLists = data1 }, err => { console.log(err) })
  }

  addToWishList(prodId: number) {
    this.us.addWishlist("harsha", prodId).subscribe(data => {
      this.logger.logStatus("Product added to wishlist");
      this.getWishlists();
    },
      err => {
        console.log(err);
  ...
  removeFromWishList(prodId: number) {
    this.us.removeWishlist("harsha", prodId).subscribe(data => {
      
      this.logger.logStatus("Product Removed from wishlist");
      this.getWishlists();
    },
      err => {
        console.log(err);
  ...
  removeProductFromCart(prodId: number) {
    this.cart.removeProductFromCart(prodId).subscribe(data => {
      this.logger.logStatus("Product Removed from cart");
    },
      err => {
        console.log(err);
      })
  ...
  getCartProducts() {
    this.listService.getCustomerCart("harsha").subscribe(data => {
      ...
      this.logger.logStatus("Received all the cart products");
      this.carts=data;
    },
      err => {
        console.log(err.stack)
