...
export class RaavanTeamService {
  ...
  getProduct() {
   ...
   placeOrder(username: string, productId: number[], quantity: number[], price: number, addrId: number) {
     return this.http.get<number>(this.baseUrl + "/placeOrder/" + "harsha" + "/" + productId + "/" + quantity + "/" + price + "/" + addrId);
   }
  ...
  clearCart(){
    return this.http.delete(this.baseUrl + "/clearCart/harsha");
  }
