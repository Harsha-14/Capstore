...
public class CapstoreDaoImpl implements CapstoreDao{
	...
	public List<Product> getSearchProducts(String prod_name)
	{
		...
		ArrayList<Product> allProducts=new ArrayList<Product>();
		       ...
		       for(int i=0;i<allProducts.size();i++) {
			      String[] words =allProducts.get(i).getProductName().split(" ");
			      if( allProducts.get(i).getProductName().substring(0,Math.min(prod_name.length(),allProducts.get(i).getProductName().length())).toLowerCase().equals(prod_name.substring(0,Math.min(prod_name.length(),allProducts.get(i).getProductName().length())).toLowerCase())
			    		  || allProducts.get(i).getProductBrand().substring(0,Math.min(prod_name.length(),allProducts.get(i).getProductBrand().length())).toLowerCase().equals(prod_name.substring(0,Math.min(prod_name.length(),allProducts.get(i).getProductBrand().length())).toLowerCase()))
			      {
			      ...
			      else
			      {
				      ...
				      {
					      if(words[j].substring(0,Math.min(prod_name.length(),words[j].length())).toLowerCase().equals(prod_name.toLowerCase().substring(0,Math.min(prod_name.length(),words[j].length()))))
					      {
					          products.add(allProducts.get(i));
					          break;
					      }  
	...
	public void clearCart (String username) {
		String cmd = " select id from Cart where username =: user";
		TypedQuery<Integer> query = entityManager.createQuery(cmd, Integer.class);
		query.setParameter("user", username);
		List<Integer> list = query.getResultList();
		for( int i=0; i<list.size(); i++) {	
			 Cart cart = entityManager.find(Cart.class, (int) list.get(i));
			 entityManager.remove(cart);
		}
